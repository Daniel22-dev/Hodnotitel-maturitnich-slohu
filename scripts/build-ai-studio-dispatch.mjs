#!/usr/bin/env node
// GARP 2.5.1 / essay-evaluator - build the AI Studio `app-updated` client_payload.
//
// Duvod, proc to neni inline v deploy.yml: GitHub REST API dovoluje v client_payload
// nejvyse 10 top-level vlastnosti. Inline heredoc to nesel otestovat, takze se limit
// porusil az v produkcnim behu (HTTP 422, 19 vlastnosti). Jako samostatny skript to
// pokryva trvala regrese.
//
// Release identita se proto nese v jednom vnorenem objektu `release`. Vnoreni je
// dokumentovany postup; limit se pocita jen na nejvyssi urovni.
//
...
export function buildDispatchPayload({ env = {}, live = {} } = {}) {
  ...
  if (live.status !== 'PASS') throw new Error('Live release was not verified; refusing to build a dispatch payload.');
  if (required.RELEASE_STAGE !== 'LIVE-PUBLIC-PAGES') {
    throw new Error(`Refusing to notify AI Studio about a non-live release (releaseStage=${required.RELEASE_STAGE}).`);
  }
  if (live.version !== required.APP_VERSION || live.artifactDigest !== required.ARTIFACT_DIGEST) {
    throw new Error('Verified live release does not match the exported release identity.');
  }
  ...
  const payload = {
    ...
    client_payload: {
      ...
      // kanonicke appId z manifestu a release identity se pridava vedle nej.
      ...
      // Vse ostatni je vnorene, aby se dodrzel limit 10 top-level vlastnosti.
      release: {
  ...
  if (count > MAX_TOP_LEVEL_PROPERTIES) {
    throw new Error(`client_payload has ${count} top-level properties; GitHub allows at most ${MAX_TOP_LEVEL_PROPERTIES}.`);
  }
  return payload;
}

// CLI: node scripts/build-ai-studio-dispatch.mjs <live-release.json> <out.json>
if (import.meta.url === `file://${process.argv[1]}`) {
  const [liveFile, outFile] = process.argv.slice(2);
  if (!liveFile || !outFile) {
    console.error('Usage: node scripts/build-ai-studio-dispatch.mjs <live-release.json> <out.json>');
    process.exit(2);
  }
  try {
    ...
    console.log(`Dispatch payload built: ${Object.keys(payload.client_payload).length} top-level properties.`);
  } catch (error) {
