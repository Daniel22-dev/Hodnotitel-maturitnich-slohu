...
import { runPromptBoundaryChecks } from './prompt-boundary-harness.mjs';

export async function validateSecurity({root, finding}) {
  const read=rel=>readFile(path.join(root,rel),'utf8');
  const [access,sw,contract,manual,workflow,gemini,aiCore,stateUi,release,privacy,tests]=await Promise.all([
    read('src/access-bootstrap.js'),read('src/sw.js'),read('src/js/45-evaluation-contract.js'),
    read('src/js/50-wordcount-offline.js'),read('src/js/65-evaluation-workflow.js'),read('src/js/60-evaluation-gemini.js'),
    read('vendor/ghrab-ai-core-1.0.0/ghrab-ai-core-1.0.0.js'),read('src/js/95-workflow-ui.js'),read('src/js/00-release-rubric.js'),read('src/js/30-privacy-input.js'),read('tests/test.mjs')
  ]);
  const findings=[];
  ...
  require(/finishReason[\s\S]{0,500}MAX_TOKENS[\s\S]{0,300}INVALID_OUTPUT/.test(aiCore) && /INVALID_OUTPUT/.test(workflow) && /MAX_TOKENS|output limit|výstupní limit/i.test(workflow+tests),'TRUNCATED_AI_RESPONSE','Oříznutá AI odpověď musí být rozpoznána podle finishReason/MAX_TOKENS a nesmí vytvořit falešný úspěch.');
  const promptBoundary=runPromptBoundaryChecks(root);
  require(promptBoundary.ok && promptBoundary.corpusCount>=28 && promptBoundary.attempts>=196,'PROMPT_INJECTION_BOUNDARY','Funkční prompt boundary musí izolovat celý AI-RED korpus ve všech nedůvěryhodných kanálech.',JSON.stringify({corpus:promptBoundary.corpusCount,channels:promptBoundary.channels,attempts:promptBoundary.attempts,failures:promptBoundary.failures.slice(0,3)}));
  ...
  return {findings,details:{profile:'essay-evaluator-audit-1.5',controls:9,promptBoundary:{corpus:promptBoundary.corpusCount,channels:promptBoundary.channels,attempts:promptBoundary.attempts}}};
}
