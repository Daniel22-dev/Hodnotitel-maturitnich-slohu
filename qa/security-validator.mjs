...
export async function validateSecurity({root, finding}) {
  ...
  const [access,sw,contract,manual,workflow,gemini,stateUi,release,privacy,tests]=await Promise.all([
  ...
  require(/finishReason[\s\S]{0,500}MAX_TOKENS[\s\S]{0,300}throw/.test(gemini) && /INVALID_OUTPUT/.test(workflow) && /MAX_TOKENS|output limit|výstupní limit/i.test(workflow+tests),'TRUNCATED_AI_RESPONSE','Oříznutá AI odpověď musí být rozpoznána podle finishReason/MAX_TOKENS a nesmí vytvořit falešný úspěch.');
